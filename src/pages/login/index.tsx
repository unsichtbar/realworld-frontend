...
export const Login: React.FC<{}> = (props) => {
    ...
    function submitForm(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
        ...
        if(username.value && password.value && !username.errors && !password.errors) {
            alert('loggin you in wowee')
        }
    }

    return <div><form>
        <span><input type="text" value={username.value} name="username" placeholder="Username" onChange={onUsernameChange}></input>{submitted && username.errors}</span>
        <span><input type="password" value={password.value} name="password" placeholder="Password" onChange={onPasswordChange}></input>{submitted && password.errors}</span>
